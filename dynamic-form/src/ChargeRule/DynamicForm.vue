<template>
  <div class="app-container">
    <el-form :model="form" ref="formRef" :rules="rules" label-width="130px">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="计费循环模式" prop="dayChargeRuleTime">
            <el-select
              v-model="form.dayChargeRuleTime"
              placeholder="请选择计费循环模式"
            >
              <el-option
            ...
            <div class="tips-item">
              0点循环：车辆停入，不满24小时按0点重新计费。
            </div>
            <div class="tips-item">
              24小时循环：车辆停入，24小时后重新计费。
            </div>
          </el-form-item>
        </el-col>
      </el-row>
      <h3>收费标准</h3>
      <div class="basic">
        1、需要分别配置每种车辆类型工作日和非工作日的收费标准；
      </div>
      <div class="basic">2、没有配置的时段默认为免费；</div>
      <div class="basic">
        3、收费时段若跨00:00点，则请分为两段配置，如19:00-07:00，则请分别配置19:00-23:59和00:00-07:00”。
      </div>
      <!-- 外部表格 -->
      <el-table v-loading="loading" :data="chargeList">
        <el-table-column prop="dateType" label="日期类型">
          <template slot-scope="scope">{{
            scope.row.dateType == 1 ? "工作日" : "非工作日"
          }}</template>
        </el-table-column>
  ...
  methods: {
    ...
    validateField(form, index) {
      ...
      for (let item of this.$refs[form].fields) {
        console.log(item.prop.split("."), index, "pppppp");
    ...
    /** 添加 */
    handleAdd() {
      this.$refs.formDialogRef.open = true;
      this.open = true;
      this.title = "添加收费标准";
    },
    /** 添加表单中的表格 */
    handleFormAdd() {
      ...
      let index = this.innerForm.specialform.innerChargeList.length;
      const newItem = {
        startTime: undefined,
        endTime: undefined,
      ...
      this.innerForm.specialform.innerChargeList.splice(index, 0, newItem);
      this.$nextTick(() => console.log(this.innerForm, "innerForm"));
      // let index = this.specialform.innerChargeList.length - 1;
      // if (!this.validateField("specialform", index)) return;
    },
    /** 提交 */
    save() {
    ...
    //确定关闭添加、编辑时段事件
    async submitForm() {
      const valid = await this.$refs.innerForm.validate();
      const valid2 = await this.$refs.specialform.validate();
    ...
    //删除表单中的表格
    handleFormDelete(i) {
      this.specialform.innerChargeList.splice(i, 1);
    },
  },
};
</script>
